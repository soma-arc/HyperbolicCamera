...
void main( ){
  //gl_FragColor = texture2D(cameraTexture, gl_FragCoord.xy / u_iResolution);return;
  ...
  float bendX =  tilt.x;//1. * abs(sin(u_iGlobalTime));;//PI / 6.;
  ...
  for(float i = 0. ; i < sampleNum ; i++){
    ...
    //position += vec2(cos(u_iGlobalTime), 0.3 * sin(u_iGlobalTime));

    position += translate;
    position = m * position;
    int d = IIS(position);
    //gl_FragColor = texture2D(u_texture, abs( vec2( 1.) - (g_pos + texTranslate) / tile));
    //return;
